...
from timenlp.timenlp import timenlp, timenlp_gen, _match_rule
from timenlp.types import Interval, Time, Artifact
...
def test_ctparse():
    ...
    assert timenlp("gargelbabel") is None
    txt = "12.12."
    res = timenlp(txt, ts=datetime(2020, 12, 1))
    assert res
    assert res.resolution == Time(year=2020, month=12, day=12)

    gres = timenlp_gen(txt, ts=datetime(2020, 12, 1))
    first_res = next(gres)
    assert first_res
...
def test_latent_time():
    parse = timenlp("8:00 pm", ts=datetime(2020, 1, 1, 7, 0), latent_time=False)
    assert parse
    assert parse.resolution == Time(None, None, None, 20, 00)

    parse = timenlp("8:00 pm", ts=datetime(2020, 1, 1, 7, 0), latent_time=True)
    assert parse
